        ...
        function updateState(newState) {
            ...
            if (drawView) {
                drawView.classList.remove('prize-100', 'prize-50', 'prize-25', 'prize-bonus');
                const curPrize = state.draw.currentPrize;
                if (curPrize === 100 || curPrize === '100') drawView.classList.add('prize-100');
                else if (curPrize === 50 || curPrize === '50') drawView.classList.add('prize-50');
                else if (curPrize === 25 || curPrize === '25') drawView.classList.add('prize-25');
                else if (curPrize === 'bonus') drawView.classList.add('prize-bonus');
            }
            ...
            const currentSlidesJson = JSON.stringify(state.slides || []) + '_' + JSON.stringify({
                t100: state.draw.cfgPrizes100,
                t50: state.draw.cfgPrizes50,
                t25: state.draw.cfgPrizes25
            });
            ...
            const activeEventName = state.draw.activeEvent || 'thursday';
            const activeDrawData = state.draw[activeEventName] || state.draw.thursday || state.draw;
            const isSunday = activeEventName === 'sunday';

            // Draw rendering
            prizesCountEl.innerText = activeDrawData.prizesLeft !== undefined ? activeDrawData.prizesLeft : state.draw.prizesLeft;
            ...
            // Hide 0 counts for slides
            if (c100) {
                if (activeDrawData.prizes100 === 0) c100.parentElement.style.display = 'none';
                else { c100.parentElement.style.display = 'block'; c100.innerText = activeDrawData.prizes100; }
            }
            if (c50) {
                if (activeDrawData.prizes50 === 0) c50.parentElement.style.display = 'none';
                else { c50.parentElement.style.display = 'block'; c50.innerText = activeDrawData.prizes50; }
            }
            if (c25) {
                if (activeDrawData.prizes25 === 0) c25.parentElement.style.display = 'none';
                else { c25.parentElement.style.display = 'block'; c25.innerText = activeDrawData.prizes25; }
            }

            // Render drawn numbers chronologically (first drawn to last drawn) with line connectors
            const drawn = activeDrawData.drawnNumbers || [];
            ...
            // Reverse to get chronological order (first drawn to last drawn)
            const chronological = [...displayDrawn].reverse();
            ...
            const renderDrawnNumberLine = (entry, i, arr, scale = 1.0) => {
                ...
                let style = `background: ${bg}; box-shadow: 0 0 15px ${glow}; padding: 0.5rem 1.4rem; border-radius: 0.75rem; color: #fff; font-weight: 800; font-size: ${2.6 * scale}rem; position: relative; z-index: 2; animation: popIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; flex-direction: column; align-items: center; justify-content: center; line-height: 1.1; min-width: ${5 * scale}rem;`;
                ...
                const lineHtml = hasNext ? `<div style="width: 2rem; height: 4px; background: rgba(255,255,255,0.2); margin-top: ${1.5 * scale}rem; z-index: 1;"></div>` : '';

                return `
                    <div style="display: flex; align-items: flex-start; margin-bottom: 1rem;">
                        <div ${extraClass} style="${style}">
                            <span>${sanitize(String(num))}</span>
                            <span style="font-size: ${0.8 * scale}rem; font-weight: 600; opacity: 0.8; text-transform: uppercase;">${prizeLabel}</span>
                        </div>
                        ${lineHtml}
                    </div>`;
            };
            ...
            if (horizontalList) {
                horizontalList.innerHTML = chronological.map((e, i, a) => renderDrawnNumberLine(e, i, a, 1.0)).join('');
            }
            ...
            if (winningList) {
                winningList.innerHTML = chronological.map((e, i, a) => renderDrawnNumberLine(e, i, a, 1.3)).join('');
            }

            // Update Robs Advert
            const robsPrizes = document.getElementById('robs-prizes-list');
            if (robsPrizes && state.draw.sunday) {
                let p = [];
                if (state.draw.sunday.cfgVouchersCount > 0) p.push(`<div>${state.draw.sunday.cfgVouchersCount}x $${state.draw.sunday.cfgVouchersValue} Vouchers</div>`);
                if (state.draw.sunday.cfgPrizes100 > 0) p.push(`<div>${state.draw.sunday.cfgPrizes100}x $100 Meat Trays</div>`);
                if (state.draw.sunday.cfgPrizes50 > 0) p.push(`<div>${state.draw.sunday.cfgPrizes50}x $50 Meat Trays</div>`);
        ...
        setInterval(() => {
            ...
            // Check congrats timeline transition to video
            if (state.activeView === 'congrats' || state.activeView === 'winning-numbers') {
                if (!congratsStartTime) {
                    congratsStartTime = Date.now();
                }
                const elapsed = Date.now() - congratsStartTime;
                const isTest = window.location.search.includes('testLoop=1');
                const congratsLimit = isTest ? 30 * 1000 : 30 * 60 * 1000;
                
                if (elapsed >= congratsLimit) {
                    updateState({ activeView: 'congrats-video' });
                    bc.postMessage({ type: 'STATE_UPDATE', payload: { activeView: 'congrats-video' } });
                    writeFirebase({ activeView: 'congrats-video' });
                }
            } else if (state.activeView !== 'congrats-video') {
                congratsStartTime = null;
            }

            // Post-draw alternating loop: Congrats -> Robs Info (if enabled) -> Winning Numbers (unless manual)
            if (state.activeView === 'congrats' || state.activeView === 'winning-numbers' || state.activeView === 'view-robs-advert') {
                if (!postDrawLoopInterval) {
                    let steps = ['congrats'];
                    if (state.draw.sunday && state.draw.sunday.enabled) {
                        steps.push('view-robs-advert');
                    }
                    const activeDrawData = state.draw[state.draw.activeEvent || 'thursday'] || {};
                    if (!activeDrawData.manualDraw) {
        ...
        function renderSlides(slidesData) {
            ...
            const t100 = state.draw.cfgPrizes100 !== undefined ? state.draw.cfgPrizes100 : 27;
            const t50 = state.draw.cfgPrizes50 !== undefined ? state.draw.cfgPrizes50 : 19;
            const t25 = state.draw.cfgPrizes25 !== undefined ? state.draw.cfgPrizes25 : 24;
            const totalPrizes = t100 + t50 + t25;
            const totalPool = (t100 * 100) + (t50 * 50) + (t25 * 25);
            
            const nzDate = getNZDate();
            const day = nzDate.getDay();
            const targetDay = (window.MMR_CONFIG && window.MMR_CONFIG.TARGET_DAY) ?? 4;

            const ticketsWording = (day === targetDay) ? "TONIGHT FROM 5:30PM" : (day === (targetDay - 1 + 7) % 7) ? "TOMORROW FROM 5:30PM" : "THURSDAYS FROM 5:30PM";
            const drawWording = (day === targetDay) ? "TONIGHT AT 7:00PM" : (day === (targetDay - 1 + 7) % 7) ? "TOMORROW AT 7:00PM" : "THURSDAYS AT 7:00PM";
            const poolWording = (day === targetDay) ? "tonight's" : (day === (targetDay - 1 + 7) % 7) ? "tomorrow's" : "Thursday's";
            ...
            const advertSlides = [
                ...
                {
                    title: `TOTAL PRIZES: ${totalPrizes}`,
                ...
                {
                    title: `TOTAL PRIZE POOL: $${totalPool.toLocaleString('en-NZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                    subtitle: `🔥 in ${poolWording} Monster Meat Raffle Draw! 🔥`,
                    type: "advert",
                    advertType: "pool",
                    duration: 30000
                },
            ...
            slidesWrapper.innerHTML = finalSlides.map((slide, i) => {
                ...
                const avatarImg = safeUrl(slide.avatarImage || slide.imageUrl || slide.backgroundImage); // For avatar type
                        ...
                        ${slide.type === 'countdown' 
                            ? `<div class="slide-countdown slide-countdown-timer" style="position: relative; z-index: 2;">${timeHtml}</div>
                               <div class="countdown-prizes-summary" style="position: relative; z-index: 2; margin-top: 3.5rem; display: flex; gap: clamp(1.5rem, 4vw, 5.5rem); justify-content: center; font-size: clamp(1.6rem, 3.8vw, 3.8rem); font-weight: bold; background: rgba(0,0,0,0.85); padding: 1.5rem clamp(1.5rem, 4vw, 4rem); border-radius: 2rem; border: 2px solid rgba(255,255,255,0.2); box-shadow: 0 15px 35px rgba(0,0,0,0.6), 0 0 25px rgba(255,255,255,0.05); max-width: 90%; margin-left: auto; margin-right: auto; box-sizing: border-box;">
                                 <span style="color: #ef4444; text-shadow: 0 0 15px rgba(239, 68, 68, 0.85), 0 0 30px rgba(239, 68, 68, 0.45);"><span id="count-100">${state.draw.prizes100 !== undefined ? state.draw.prizes100 : 27}</span> x $100</span>
                                 <span style="color: #3b82f6; text-shadow: 0 0 15px rgba(59, 130, 246, 0.85), 0 0 30px rgba(59, 130, 246, 0.45);"><span id="count-50">${state.draw.prizes50 !== undefined ? state.draw.prizes50 : 19}</span> x $50</span>
                                 <span style="color: #10b981; text-shadow: 0 0 15px rgba(16, 185, 129, 0.85), 0 0 30px rgba(16, 185, 129, 0.45);"><span id="count-25">${state.draw.prizes25 !== undefined ? state.draw.prizes25 : 24}</span> x $25</span>
                               </div>` 
                            : ''
                        }
        ...
        function parseSlidesCSV(text) {
            ...
            return result.slice(1).map(clean => {
                ...
                if (type === 'avatar') return null; // Ignore Google Sheets avatar slides completely
        ...
        async function fetchAndApplySlides() {
            ...
            if (!success) {
                try {
                    ...
                    console.error('[MMR] Failed to load local-backup.csv:', e);
                }
            }
