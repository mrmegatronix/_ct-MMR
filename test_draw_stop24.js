...
(async () => {
    try {
        ...
        // Let's print out what is returned by document.querySelectorAll('.slot-digit') inside the actual page BEFORE stopping
        const preStopLen = await page.evaluate(() => {
            return document.querySelectorAll('.slot-digit').length;
        });
        console.log(`Number of .slot-digit elements BEFORE stopping: ${preStopLen}`);
        ...
        const postStopLen = await page.evaluate(() => {
            return document.querySelectorAll('.slot-digit').length;
        });
        console.log(`Number of .slot-digit elements RIGHT AFTER stopping: ${postStopLen}`);
