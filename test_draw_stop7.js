    ...
    try {
        ...
        await page.evaluate(() => {
             if (typeof window.stopLotteryRoll === 'function') {
                ...
                window._testTimeouts = [];
                const origSetTimeout = window.setTimeout;
                window.setTimeout = function(cb, delay) {
                    const id = origSetTimeout(cb, delay);
                    window._testTimeouts.push({id, delay});
                    return id;
                };
        ...
        // Let's poll for the rolling class to disappear
        for (let i = 0; i < 20; i++) {
            await new Promise(r => setTimeout(r, 200));
            const hasRolling = await page.evaluate(() => {
                return document.querySelector('.slot-digit.rolling') !== null;
            });
            if (!hasRolling) break;
        }
        ...
        const lockTimeouts = await page.evaluate(() => window.lockTimeouts);
        console.log("lockTimeouts array length:", lockTimeouts.length);
